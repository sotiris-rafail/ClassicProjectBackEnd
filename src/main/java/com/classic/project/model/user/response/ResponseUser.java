...
public class ResponseUser {
    ...
    public static ResponseUser convert(Optional<User> user){
        ...
        return new ResponseUser(user.get().getEmail(), user.get().getTypeOfUser().name(), ResponseCharacter.convert(user.get().getCharacters()), ResponseConstantParty.convert(user.get().getCp()));
    }
}
