...
public class MessageListener extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (!event.getAuthor().isBot()) {
            ...
            if (checkForDelay(event)) {
                ...
                displayNextExecutableTIme.setTime(new Date(new Date(event.getMessage().getTimeCreated().toInstant().toEpochMilli()).getTime() - getPreviousCommandTime(retrievedHistory.get(getMessageIndex(getCommand(event.getMessage().getContentRaw()))))));
                textChannel.sendMessage(String.format("Previous %s command was executed %2$s minutes and %3$s seconds ago. 5 minutes are required to pass before you can run the same command.", getCommand(event.getMessage().getContentRaw()), displayNextExecutableTIme.get(Calendar.MINUTE),  displayNextExecutableTIme.get(Calendar.SECOND))).queue();
                return;
            }
    ...
    private boolean checkForDelay(MessageReceivedEvent event) {
        ...
        try {
            ...
            x = getMessageIndex(getCommand(event.getMessage().getContentRaw()));
            if(x == -1) {
            	return false;
			}
            if (getCommand(retrievedHistory.get(x).getContentRaw()).equalsIgnoreCase(getCommand(event.getMessage().getContentRaw()))) {
                if ((new Date().getTime() - getPreviousCommandTime(retrievedHistory.get(x))) >= MAX_DURATION) {
                    return false;
                }
            } else {
                return false;
    ...
    private int getMessageIndex(String command) {
        for (Message message : retrievedHistory) {
            if (getCommand(message.getContentRaw()).startsWith(command)) {
                return retrievedHistory.indexOf(message);
            }
        }
        return -1;
    }

    private long getPreviousCommandTime(Message retrievedMessage) {
        return new Date(retrievedMessage.getTimeCreated().toInstant().toEpochMilli()).getTime();
    }
